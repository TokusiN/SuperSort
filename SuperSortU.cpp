...
namespace {
	void SuperSortAligned(unsigned int* array, size_t num);
	void SuperSort64(unsigned int* array, unsigned int* dst = NULL);
	void SuperSort96(unsigned int* array, unsigned int* dst = NULL);
	void SuperSort128(unsigned int* array, unsigned int* dst = NULL);
} // namespace

void SuperSort(unsigned int* array, size_t num)
{
	...
	{
		unsigned int* buf = (unsigned int*)AlignedMalloc(sizeof(unsigned int) * alignedsize);
		size_t i;
		for (i = num; i < alignedsize; i++)
		{
			buf[i] = 0xffffffff;
		}
		memcpy(buf, array, sizeof(unsigned int) * num);
		if (alignedsize > 128)
		{
		...
		memcpy(array, buf, sizeof(unsigned int) * num);
		AlignedFree(buf);
	}
}
...
namespace {
	...
	auto Comparator = [](__m256i& lo, __m256i& hi) {
		__m256i t;
		t = _mm256_min_epu32(lo, hi);
		hi = _mm256_max_epu32(lo, hi);
		lo = t;
	};
...
#define LineBitonicSort() \
	...
	auto ComparatorLR2 = [](__m256i& m0, __m256i& m1) {
		...
		mu = _mm256_max_epu32(mt, ms);
		mt = _mm256_min_epu32(mt, ms);
	...
	auto ComparatorLR = [](__m256i& m) {
		...
		mt = _mm256_min_epu32(m, ms);
		m = _mm256_max_epu32(m, ms);
...
#define Merge3232() {\
	...
	void SuperSort64(unsigned int* array, unsigned int* dst)
	{
	...
	void SuperSort96(unsigned int* array, unsigned int* dst)
	{
		if (!dst)
		{
			dst = array;
	...
	void SuperSort128(unsigned int* array, unsigned int* dst)
	{
		if (!dst)
		{
			dst = array;
	...
	void Merge(unsigned int* src1, size_t size1, unsigned int* src2, size_t size2, unsigned int* dst)
	{
		unsigned int i, j;
		i = j = 1;
		__m256i m0, m1, m2, m3, m4, m5, m6, m7;
	...
	void SuperSortRec(unsigned int* src, unsigned int* dst, unsigned int* org, size_t num)
	{
		if (num > 4)
		{
	...
	void SuperSortAligned(unsigned int* array, size_t num)
	{
		unsigned int* buf = (unsigned int*)AlignedMalloc(sizeof(unsigned int) * num * 32);
		size_t i;
